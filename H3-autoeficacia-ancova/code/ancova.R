...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "autoeficacia.pre", ANCOVA tests with independent between-subjects variables "condicao" (inBoost, inThreat, neutro) were performed to determine statistically significant difference on the dependent varibles "autoeficacia.pos". For the dependent variable "autoeficacia.pos", there was statistically significant effects in the factor "autoeficacia.pre" with F(1,137)=79.814, p<0.001 and ges=0.368 (effect size) and in the factor "condicao" with F(2,137)=5.947, p=0.003 and ges=0.08 (effect size).
#' 
#' 
#' Pairwise comparisons using the Estimated Marginal Means (EMMs) were computed to find statistically significant diferences among the groups defined by the independent variables, and with the p-values ajusted by the method "bonferroni". For the dependent variable "autoeficacia.pos", the mean in the condicao="inBoost" (adj M=1.488 and SD=0.275) was significantly different than the mean in the condicao="neutro" (adj M=1.69 and SD=0.407) with p-adj=0.002.
#' 
#' 
#' 
#' ## Tips and References
