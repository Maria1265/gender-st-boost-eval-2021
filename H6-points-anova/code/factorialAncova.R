...
#' ### Textual Report
#' 
#' ANOVA tests with independent between-subjects variables "stType" (default, stFemale, stMale) and "gender" (Feminino, Masculino) were performed to determine statistically significant difference on the dependent varibles "points". For the dependent variable "points", there was statistically significant effects in the factor "stType" with F(2,116)=3.786, p=0.026 and ges=0.061 (effect size) and in the factor "gender" with F(1,116)=15.361, p<0.001 and ges=0.117 (effect size).
#' 
#' 
#' Pairwise comparisons using the Estimated Marginal Means (EMMs) were computed to find statistically significant diferences among the groups defined by the independent variables, and with the p-values ajusted by the method "bonferroni". For the dependent variable "points", the mean in the stType="stFemale" (adj M=2.921 and SD=0.486) was significantly different than the mean in the stType="stMale" (adj M=2.51 and SD=0.623) with p-adj=0.031; the mean in the gender="Feminino" (adj M=3.098 and SD=0.726) was significantly different than the mean in the gender="Masculino" (adj M=2.51 and SD=0.623) with p-adj<0.001.
#' 
#' 
#' 
#' ## Tips and References
